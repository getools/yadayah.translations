<?php
/**
 * One-off: re-sweep already-built TTS audio so paragraphs carrying the words
 * "Gowym", "metanoeo" or "Peter" re-render with the current pronunciation
 * handling. For every active audio row (complete or paused), find paragraphs
 * whose plain text contains any of those whole words, delete their positional
 * cached part files so they re-synth, and (for 'complete' rows, with --apply)
 * flip the chapter to 'pending' so the build watchdog rebuilds it -- reusing
 * every unaffected cached part.
 *
 *   php _wordfix_resweep.php                 # dry-run, all rows
 *   php _wordfix_resweep.php --ak=NNN        # dry-run, one audio key
 *   php _wordfix_resweep.php --apply         # delete parts + reflag complete
 */
require_once __DIR__ . '/config.php';
require_once __DIR__ . '/admin-tts-helpers.php';

$WORD_RE = '/\b(?:Gowym|metanoeo|Peter)\b/i';
...
foreach ($rows as $a) {
    ...
    // Paragraphs carrying one of the target words (whole-word, case-insensitive).
    $affIdx = []; $affParaCount = 0;
    foreach ($prows as $r) {
        $plain = (string)$r['paragraph_text_plain'];
        if ($plain === '') continue;
        if (preg_match($WORD_RE, $plain)) {
            $num = (int)$r['paragraph_number'];
        ...
        if ($status === 'complete') {
            $db->prepare("UPDATE yy_tts_audio
                             SET tts_audio_status='pending', tts_audio_worker_pid=NULL,
                                 tts_audio_message='re-queued: word pronunciation resweep (Gowym/metanoeo/Peter)'
