<?php
/**
 * One-off: re-sweep already-built TTS audio for clock-time mispronunciations
 * now fixed in rewriteClockTimes() ("6:22 PM" was read "six thousand twenty
 * two"). Same positional-part-index technique as _islam_resweep.php: for every
 * active audio row built with the OLD code (complete or paused), find the
 * paragraphs whose synthesized text now changes, delete their cached
 * p%05d.mp3 parts so they re-synth, and (only for 'complete' rows, with
 * --apply) flip the chapter to 'pending' for the build watchdog.
 *
 * Detection uses rewriteClockTimes($plain, true) — the local-engine ($spell)
 * form, since Azure is untouched by the fix. So this only flags chapters whose
 * text actually changes on the self-hosted engines (the ones that mangle it).
 *
 *   php _clocktime_resweep.php                 # dry-run, all rows
 *   php _clocktime_resweep.php --ak=NNN        # dry-run, one audio key
 *   php _clocktime_resweep.php --apply         # delete parts + reflag complete
 */
...
foreach ($rows as $a) {
    ...
    // Which paragraphs' synthesized text changes under the clock-time rule?
    $affIdx = []; $affParaCount = 0;
    foreach ($prows as $r) {
        $plain = (string)$r['paragraph_text_plain'];
        if ($plain === '') continue;
        if (rewriteClockTimes($plain, true) !== $plain) {
            $num = (int)$r['paragraph_number'];
    ...
    printf("ak=%-5d st=%-8s chap=%-6d times=%-3d part_del=%-3d absent=%d\n",
        ...
        if ($status === 'complete') {
            $db->prepare("UPDATE yy_tts_audio
                             SET tts_audio_status='pending', tts_audio_worker_pid=NULL,
                                 tts_audio_message='re-queued: clock-time pronunciation fix (resweep)'
