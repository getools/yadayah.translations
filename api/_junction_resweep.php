<?php
/**
 * One-off resweep for the glyph-drop junction cleanup (see
 * reference_tts_font_skip_embedded_names — dropped-glyph seams). For every chapter whose
...
$map = json_decode((string)file_get_contents(__DIR__ . '/_junction_map.json'), true);
        ...
        if ($status === 'complete') {
            $db->prepare("UPDATE yy_tts_audio
                             SET tts_audio_status='pending', tts_audio_worker_pid=NULL,
                                 tts_audio_message='re-queued: glyph-drop junction cleanup (resweep)'
