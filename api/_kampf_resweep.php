<?php
/**
 * One-off: re-sweep already-built TTS audio so Mein Kampf citations re-render
 * with the new 'adolph' voice (German-blend model, α0.60). For every active
 * audio row (complete or paused), find paragraphs carrying a Mein Kampf
 * reference (data-style kampf / mein_kampf), delete their positional cached
 * part files so they re-synth with the kampf-category voice (now Adolph), and
 * (for 'complete' rows, with --apply) flip the chapter to 'pending' so the
 * build watchdog rebuilds it — reusing every unaffected cached part.
 *
 *   php _kampf_resweep.php                 # dry-run, all rows
 *   php _kampf_resweep.php --ak=NNN        # dry-run, one audio key
 *   php _kampf_resweep.php --apply         # delete parts + reflag complete
 */
...
foreach ($rows as $a) {
    ...
    // Paragraphs carrying a Mein Kampf reference (kampf / mein_kampf data-style).
    $affIdx = []; $affParaCount = 0;
    foreach ($prows as $r) {
        $html = (string)$r['paragraph_text_html'];
        if ($html === '') continue;
        if (preg_match('/data-style\s*=\s*["\']?(mein_)?kampf["\']?/i', $html)) {
            $num = (int)$r['paragraph_number'];
    ...
    printf("ak=%-5d st=%-8s chap=%-6d kampf=%-3d part_del=%-3d absent=%d\n",
        ...
        if ($status === 'complete') {
            $db->prepare("UPDATE yy_tts_audio
                             SET tts_audio_status='pending', tts_audio_worker_pid=NULL,
                                 tts_audio_message='re-queued: Mein Kampf voice -> Adolph (resweep)'
