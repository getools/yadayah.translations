<?php
/**
 * One-off: re-sweep already-built TTS audio for bare three-digit numbers now
 * fixed in rewriteBareNumbers() — Chatterbox read "610" as "six hundred",
 * dropping the tens place (reported on YY-s07v01 page 9). Same
 * positional-part-index technique as _clocktime_resweep.php / _islam_resweep.php:
 * for every active audio row built with the OLD code (complete or paused), find
 * the paragraphs whose synthesized text now changes, delete their cached
 * p%05d.mp3 parts so they re-synth, and (only for 'complete' rows, with
 * --apply) flip the chapter to 'pending' for the build watchdog.
 *
 * Detection runs the citation and clock rewrites FIRST, then asks whether
 * rewriteBareNumbers() changes anything further. That ordering matters: it
 * mirrors buildLocalSegment(), so a paragraph whose only three-digit numbers
 * sit inside a citation ("Ishaq:315") — already spelled by the citation rule,
 * and already re-swept — is correctly NOT flagged again here.
 *
 ...
 *
 *   php _numfix_resweep.php                 # dry-run, all rows
 *   php _numfix_resweep.php --ak=NNN        # dry-run, one audio key
 *   php _numfix_resweep.php --apply         # delete parts + reflag complete
 */
...
/** Does the new bare-number rule change this paragraph's synthesized text? */
        ...
        if ($status === 'complete') {
            $db->prepare("UPDATE yy_tts_audio
                             SET tts_audio_status='pending', tts_audio_worker_pid=NULL,
                                 tts_audio_message='re-queued: three-digit number pronunciation fix (resweep)'
